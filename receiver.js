...
const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
        try {
            ...
            console.log(`[webhooks.email] ${new Date().toLocaleTimeString()} — wrote ${safeFilename} (${content.length}B)`);
...
server.listen(PORT, '0.0.0.0', () => {
    console.log(`[webhooks.email] Security bridge active on http://0.0.0.0:${PORT}`);
    console.log(`[webhooks.email] Awaiting incoming vibe-code payloads from network...`);
});
